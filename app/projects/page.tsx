...
export default async function ProjectsPage() {
  ...
  try {
    if (publishedProjects.length > 0) {
      ...
      const viewData = await Promise.race([
        ...
        new Promise<number[]>((resolve) => setTimeout(() => resolve([]), 800)), // 800ms timeout
      ]);
      views = viewData.reduce(
        (acc, v, i) => {
  ...
  const featured =
    publishedProjects.find((project) => project.slug === "superhandlers") ||
    publishedProjects[0];

  const top2 =
    publishedProjects.find((project) => project.slug === "hindutravell") ||
    publishedProjects[1];

  const top3 =
    publishedProjects.find((project) => project.slug === "tummoc") ||
    publishedProjects[2];

  // Ensure we have unique projects for featured section
  const featuredSlugs = new Set([featured?.slug, top2?.slug, top3?.slug].filter(Boolean));
